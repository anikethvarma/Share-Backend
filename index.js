...
app.get("/api/all-meetings", async (req, res) => {
  try {
    await client.connect();
...
app.get("/api/all-refferals", async (req, res) => {
  try {
    await client.connect();
...
app.post("/api/register", async (req, res) => {
  try {
    ...
    if (member == [] || member == null) {
      ...
      res.send(JSON.stringify("Email Already Exists. Please Login"));
    }
  } catch (error) {
    res.status(500).send(error.message);
  } finally {
...
app.get("/api/members/mydetails", authenticateToken, async (req, res) => {
  try {
    await client.connect();
    const { email } = req;
    const database = client.db("app-database");
...
app.put("/api/members/mydetails", authenticateToken, async (req, res) => {
  try {
    await client.connect();
    const { email } = req;
    // const { name, surname, gothram, caste, category, age, marital_status, address, bussiness, blood_group, mla_constituency, bussiness_profile, phone_number, aadhar_number, referrence_emailId, area_location, any_service_activiy } = req.body;
    const database = client.db("app-database");
    const collection = database.collection("members");
    const refferals = await collection.updateOne({ email: email }, { $set: req.body });
...
app.get("/api/refferals/myrefferals", authenticateToken, async (req, res) => {
  try {
    await client.connect();
    const { email } = req;
    const database = client.db("app-database");
    const collection = database.collection("refferals");
    const refferals = await collection.findOne({
      $or: [{ fromMemberId: email }, { toMemberId: email }],
    });
    res.json(refferals);
  } catch (error) {
    res.status(500).send(error.message);
...
app.post("/api/addrefferal", authenticateToken, async (req, res) => {
  try {
    await client.connect();
    const { email } = req;
    ...
    const database = client.db("app-database");
    const collection = database.collection("refferals");
    const refferals = await collection.insertOne({
      fromMemberId: email,
      toMemberId: refferedId,
    });
    res.status(200).send(JSON.stringify("Refferal Added Successfully"));
  } catch (error) {
    res.status(500).send(error.message);
  } finally {
    await client.close();
